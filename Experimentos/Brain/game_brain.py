...
import logging
from fed_brain import Federation
...
logging.basicConfig(level=logging.INFO, filename="results_brain.log",format="%(asctime)s-%(levelname)s-%(message)s")
logger = logging.getLogger()



class games(Federation):
    def __init__(self, input_channels, num_classes, dict_clients_dataa):
        super().__init__(input_channels, num_classes, dict_clients_dataa)
    ...
    def coalitional_values(self,iteration,testing_loader, epochs=5):
        """Compute the evaluation of each coalition from  scratch"""
        d = dict()
        d[()]=0
    ...
    def combine_plot(self,groups=list,categories=list,name=str):
       ...
       # Labels, legend, and tick marks
        ...
        plt.legend()
        plt.grid()
        plt.savefig(f'/Users/delio/Documents/Working projects/Balazs/Experiments/Brain/figures_brain/{name}')
...
if __name__ == "__main__":
    ...
    case="Case_10_iter"
    ###################### data case
    dic_labels={0:[100,100,0,0],1:[0,100,100,0],2:[0,0,100,100]}
    clientes=data_for_clients(dic_labels)
    ######common dataset
    common_dataset=commun_test_set(clientes)
    #####game instance
    juegos=games(input_channels,num_classes,clientes)
    ######shapley retraining
    values_re=juegos.coalitional_values(10,common_dataset)
    transform_re=juegos.transform_dict_to_lists(values_re,3)
    shapley_re=juegos.shapley(3,transform_re[0],transform_re[1],"SV_re")
    ######shapley at round
    values_ro=juegos.coalitional_values_II(10,common_dataset)
